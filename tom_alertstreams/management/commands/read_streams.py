...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        #TODO: these arguments are appropriate for a single alert stream but need to be generalized for all configured streams.
    ...
    def _get_hop_authentication(self, options):
        ...
        Otherwise, get SCIMMA_AUTH_USERNAME and SCIMMA_AUTH_PASSWORD from environment.
        """
        ...
        if username is None or password is None:
            error_message = 'Supply SCiMMA Auth Hopskotch credentials on command line or set SCIMMA_AUTH_USERNAME and SCIMMA_AUTH_PASSWORD environment variables.'
    ...
    def _test_sys_heartbeat(self, auth):
        ...
        with stream.open(f'kafka://kafka.scimma.org/{topic}', 'r') as src:
            ...
            for heartbeat, metadata in src.read(metadata=True):
                self._heartbeat_handler(heartbeat, metadata)
